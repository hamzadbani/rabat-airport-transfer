...
@endphp
<div id="reserver" class="rounded-2xl border border-white/10 bg-[#ffeded6e] p-5 text-white shadow-2xl sm:p-6">
    <h2 class="text-lg font-semibold text-white">{{ __('site.booking.title') }}</h2>
    <p class="mt-1 text-sm text-white/80">{{ __('site.booking.subtitle') }}</p>

    <div class="mt-4 flex flex-wrap gap-2">
        @foreach ($presets as $preset)
            <button type="button"
                class="preset-route rounded-full border border-white/25 bg-white/10 px-3 py-1 text-xs font-medium text-white hover:border-brand hover:bg-white/20"
    ...
    <form action="{{ route('book.store') }}" method="POST" class="mt-5 space-y-4" id="booking-form">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <div>
                <label for="client_name" class="mb-1 block text-sm font-medium text-white">{{ __('site.booking.name') }}</label>
            ...
            <div>
                <label for="phone" class="mb-1 block text-sm font-medium text-white">{{ __('site.booking.phone') }}</label>
                <input required type="tel" name="phone" id="phone" value="{{ old('phone') }}" placeholder="+212 6…"
                    class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm outline-none ring-brand focus:ring-2">
            </div>
        </div>
        <div>
            <label for="pickup_location" class="mb-1 block text-sm font-medium text-white">{{ __('site.booking.departure') }}</label>
            <input required name="pickup_location" id="pickup_location" value="{{ old('pickup_location', $defaultFrom) }}"
                class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm outline-none ring-brand focus:ring-2">
        </div>
        <div>
            <label for="dropoff_location" class="mb-1 block text-sm font-medium text-white">{{ __('site.booking.arrival') }}</label>
        ...
        <div class="grid gap-4 sm:grid-cols-3">
            <div>
                <label for="date" class="mb-1 block text-sm font-medium text-white">{{ __('site.booking.date') }}</label>
                <input required type="date" name="date" id="date" value="{{ old('date', now()->format('Y-m-d')) }}"
                    class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm outline-none ring-brand focus:ring-2">
            </div>
            <div>
                <label for="time" class="mb-1 block text-sm font-medium text-white">{{ __('site.booking.time') }}</label>
                <input required type="time" name="time" id="time" value="{{ old('time', '12:00') }}"
                    class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm outline-none ring-brand focus:ring-2">
            </div>
            <div>
                <label for="passengers" class="mb-1 block text-sm font-medium text-white">{{ __('site.booking.passengers') }}</label>
                <input type="number" name="passengers" id="passengers" min="1" max="50" value="{{ old('passengers', 1) }}"
                    class="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm outline-none ring-brand focus:ring-2">
            </div>
        </div>
        <div>
            <label for="flight_number" class="mb-1 block text-sm font-medium text-white">{{ __('site.booking.flight') }} <span class="text-white/60">({{ __('site.booking.optional') }})</span></label>
